...
export function HeroApiMoment() {
  ...
  // Live response state — Send replaces the engine-port values with the real API's.
  const [res, setRes] = React.useState(RESULT);
  const [latencyMs, setLatencyMs] = React.useState<number | null>(null);
  const replayTimers = React.useRef<ReturnType<typeof setTimeout>[]>([]);

  const sendBusy = phase === "sending" || phase === "streaming";
  ...
  function handleSend() {
    ...
    setAmount(0);
    setPhase("idle");
    // Real round-trip: the settled panel then shows the live /v1/rates numbers.
    calculatePublicRate({ origin: "400001", destination: "110001", weightGrams: 1200, service: "express", cod: false })
      .then(({ result, engineMs }) => {
        setRes(result);
        setLatencyMs(engineMs ?? null);
      })
      .catch(() => {
        /* API unreachable → keep the engine-port values (identical math) */
      });
    replayTimers.current = [
      setTimeout(() => setPhase("sending"), 60),
      setTimeout(() => setPhase("streaming"), 720),
      setTimeout(() => setPhase("settled"), 1470),
    ];
  }
  React.useEffect(() => () => replayTimers.current.forEach(clearTimeout), []);
  ...
  // Count-up once the response settles (targets the latest — possibly live — total).
  React.useEffect(() => {
    if (phase !== "settled") return;
    const controls = animate(0, res.total, {
      duration: reduce ? 0 : 0.7,
      ease: "easeOut",
      onUpdate: (v) => setAmount(v),
    });
    return () => controls.stop();
  }, [phase, reduce, res.total]);
        ...
        >
          ...
          >
            <PanelHeader>
              ...
              <button
                ...
                className="group ml-auto inline-flex items-center gap-1.5 rounded-md border border-border bg-background px-2.5 py-1 text-[11px] font-semibold transition-colors hover:bg-accent disabled:opacity-60"
              >
                <Icon
          ...
          >
            <PanelHeader>
              ...
              >
                served in {latencyMs != null ? Math.max(1, Math.round(latencyMs)) : 11} ms
              </span>
              ...
              <div className="mt-1 flex flex-wrap items-end gap-x-3 gap-y-2">
                ...
                <div className="flex items-center gap-1.5 pb-1">
                  ...
                  <Chip show={settled} reduce={reduce} testId="hero-eta-chip">
                    ETA {res.etaDays[0]}-{res.etaDays[1]} days
                  </Chip>
                </div>
              </div>
              <span className="sr-only">
                Postpin returns {formatCurrency(res.total)} for a 1.2 kg express parcel from
                Mumbai 400001 to Delhi 110001, {res.zoneLabel} zone, {res.etaDays[0]} to{" "}
                {res.etaDays[1]} days.
              </span>
            </div>
            ...
            >
              {[
                <Ln key="o">{"{"}</Ln>,
                <Ln key="t" indent><K>"total"</K>: <N>{res.total.toFixed(2)}</N>,</Ln>,
                <Ln key="c" indent><K>"currency"</K>: <S>"INR"</S>,</Ln>,
                <Ln key="z" indent><K>"zone"</K>: <S>"{res.zone}"</S>,</Ln>,
                <Ln key="e" indent><K>"eta_days"</K>: <N>[{res.etaDays[0]}, {res.etaDays[1]}]</N>,</Ln>,
                <Ln key="b" indent><K>"breakdown"</K>: <span className="text-muted-foreground">[ {res.breakdown.map((l) => `${l.label.split(" ")[0]!.toLowerCase()} ${l.amount.toFixed(2)}`).join(", ")} ]</span>,</Ln>,
                <Ln key="m" indent><K>"meta"</K>: <span className="text-muted-foreground">{"{"} <K>"engine"</K>: <S>"live"</S> {"}"}</span></Ln>,
                <Ln key="x">{"}"}</Ln>,
              ].map((line) => (
